 ...
 */
function calculateTrajectoryBounds(origin, direction, minY, gravity, outBox) {
    ...
    outBox.max.set(Math.max(origin.x, endX), Math.max(origin.y, peakY), Math.max(origin.z, endZ));
    return outBox;
}
function sampleParabolicCurve(start, direction, minY, gravity, points, offset = 0) {
    const a = 0.5 * gravity;
    const b = direction.y;
...
export { calculateTrajectoryBounds, sampleParabolicCurve };
