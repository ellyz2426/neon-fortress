...
function generateSceneProceduralTexture(channel, descriptor) {
    const [width, height] = descriptor.resolution;
    const field = generatePeriodicField(descriptor, width, height);
    const data = channel === 'albedo' || channel === 'emissive'
        ? generateColorBytes(field, descriptor)
        : channel === 'normal'
            ? generateNormalBytes(field, width, height, descriptor)
            : generateScalarBytes(field, descriptor);
    return {
        channel,
        data,
        dataHash: sha256Bytes(data),
        height,
        recipeHash: sha256(canonicalizeJson({ channel, descriptor })),
        width,
    };
}
function generatePeriodicField(descriptor, width, height) {
    const field = new Float32Array(width * height);
    const amplitudeSum = descriptor.bands.reduce((sum, band) => sum + band.amplitude, 0);
...
function periodicValueNoise(u, v, band, seed, bandIndex) {
    ...
    const d = latticeHash(mod(x0 + 1, frequencyU), mod(y0 + 1, frequencyV), bandSeed);
...
function generateColorBytes(field, descriptor) {
...
function generateScalarBytes(field, descriptor) {
...
function generateNormalBytes(field, width, height, descriptor) {
    const data = new Uint8Array(field.length * 4);
...
function sampleIndex(value, size, wrap) {
    if (wrap === 'clamp') {
...
export { generateSceneProceduralTexture };
//# sourceMappingURL=procedural-textures.js.map
