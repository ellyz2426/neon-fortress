...
import { validateSceneReview, validateSceneDocument } from './validation.js';
 ...
 */
function finalizeSceneReviewDraft(document, capabilityHash, draft) {
    const configuredLensIds = document.authoring?.composition?.review.lenses;
    const requestedLenses = new Map(draft.lenses.map((lens) => [lens.id, cloneReviewLens(lens)]));
    const lensIds = configuredLensIds ?? draft.lenses.map((lens) => lens.id);
    const lenses = lensIds.map((lensId) => requestedLenses.get(lensId) ?? {
        captures: [],
        id: lensId,
        status: 'fail',
    });
    const captures = lenses.flatMap((lens) => lens.captures);
    const defaultCapture = lenses.find((lens) => lens.id === 'final')?.captures[0] ?? captures[0];
    const visualResults = new Map((draft.visualResults ?? []).map((result) => [
    ...
    for (const feature of document.authoring?.composition?.features ?? []) {
        for (const criterion of feature.acceptance) {
            ...
            const capture = requiredView == null
                ? defaultCapture
            ...
            if (criterion.kind === 'visual-judgment') {
                const provided = visualResults.get(reviewResultKey(feature.id, criterion.id));
                featureResults.push({
            ...
            const status = evaluation.status === 'unavailable' ? 'fail' : evaluation.status;
            featureResults.push({
                criterion: criterion.id,
    ...
    const allPass = lenses.every((lens) => lens.status === 'pass') &&
    ...
    const review = {
        ...
        sourceHashes: document.authoring?.composition == null
            ? []
            : getSceneCompositionSourceHashes(document.authoring.composition.input),
        stop: allPass
            ? { openDefectTags: [], reason: 'success' }
            : {
                openDefectTags: draft.openDefectTags != null && draft.openDefectTags.length > 0
    ...
    return {
        captures: lens.captures.map((capture) => ({
            ...
            ...(capture.nodeMaskRegions == null
                ? {}
                : {
                    nodeMaskRegions: Object.fromEntries(Object.entries(capture.nodeMaskRegions).map(([id, region]) => [
                        id,
                        [...region],
                    ])),
                }),
...
function reviewResultKey(feature, criterion) {
    return `${feature}\u0000${criterion}`;
}
function validateSceneReviewAgainstDocument(value, document, expectedCapabilityHash) {
    ...
    const review = value;
    compareHash(issues, '$.documentHash', review.documentHash, hashSceneDocument(document));
    compareHash(issues, '$.runtimeHash', review.runtimeHash, hashRuntimeSceneDocument(document));
    if (expectedCapabilityHash != null) {
        compareHash(issues, '$.capabilityHash', review.capabilityHash, expectedCapabilityHash);
    }
    ...
    const views = new Map((document.authoring?.views ?? []).map((view) => [view.id, view]));
    const requiredLenses = new Set(document.authoring?.composition?.review.lenses ?? []);
    const requiredViews = new Set(document.authoring?.composition?.review.requiredViews ?? []);
    for (const lens of review.lenses) {
        ...
        if (lensIsRequired && lens.captures.length === 0) {
            addReference(issues, '$.lenses', `required lens "${lens.id}" has no captures`);
        }
        for (const capture of lens.captures) {
            requiredViews.delete(capture.view);
            const view = views.get(capture.view);
            if (view == null) {
                addReference(issues, '$.lenses', `capture "${capture.id}" references unknown view "${capture.view}"`);
            }
            ...
            capture.visibleNodeIds.forEach((nodeId) => {
                if (!nodeIds.has(nodeId)) {
                    addReference(issues, '$.lenses', `capture "${capture.id}" names unknown visible node "${nodeId}"`);
                }
            });
        }
    }
    ...
    const features = new Map((document.authoring?.composition?.features ?? []).map((feature) => [
        feature.id,
        feature,
    ]));
    const captures = new Map();
    review.lenses.forEach((lens) => lens.captures.forEach((capture) => captures.set(capture.id, capture)));
    const resolvedDocument = document;
    const results = new Set();
    review.featureResults.forEach((result, index) => {
        const feature = features.get(result.feature);
        const criterion = feature?.acceptance.find((entry) => entry.id === result.criterion);
        if (feature == null) {
            addReference(issues, `$.featureResults[${index}].feature`, `unknown feature "${result.feature}"`);
        }
        else if (criterion == null) {
            addReference(issues, `$.featureResults[${index}].criterion`, `unknown criterion "${result.criterion}" for feature "${result.feature}"`);
        }
        else if (feature.priority === 'required' &&
            result.evidenceRefs.length === 0) {
            addReference(issues, `$.featureResults[${index}].evidenceRefs`, `required criterion "${result.feature}/${result.criterion}" has no evidence`);
        }
        if (criterion != null) {
            validateFeatureResultMeasurement(resolvedDocument, criterion, result, index, captures, issues);
        }
    ...
    for (const feature of features.values()) {
        ...
        for (const criterion of feature.acceptance) {
            if (!results.has(resultKey(feature.id, criterion.id))) {
                addReference(issues, '$.featureResults', `required criterion "${feature.id}/${criterion.id}" has no result`);
            }
        }
    }
    const expectedSources = new Set(document.authoring?.composition == null
        ? []
        : getSceneCompositionSourceHashes(document.authoring.composition.input).map((hash) => hash.toLowerCase()));
    const actualSources = new Set(review.sourceHashes.map((hash) => hash.toLowerCase()));
    for (const hash of expectedSources) {
        if (!actualSources.has(hash)) {
            addReference(issues, '$.sourceHashes', `source hash "${hash}" is missing`);
        }
    }
    for (const hash of actualSources) {
        if (!expectedSources.has(hash)) {
            addReference(issues, '$.sourceHashes', `source hash "${hash}" is not declared by the scene`);
        }
    }
    return { valid: issues.length === 0, issues };
}
function validateFeatureResultMeasurement(document, criterion, result, resultIndex, captures, issues) {
    const path = `$.featureResults[${resultIndex}]`;
    const requiredView = 'view' in criterion ? criterion.view : undefined;
    const referencedCapture = requiredView == null
        ? undefined
        : result.evidenceRefs
    ...
    const expectedStatus = evaluation.status === 'unavailable' ? 'fail' : evaluation.status;
...
const CANONICAL_REVIEW_LENSES = ['layout', 'geometry', 'final'];
function validateReviewWorkflow(review, configuration, issues) {
    ...
    if (!configurationIsCanonical) {
        issues.push({
            code: 'state',
            path: '$.lenses',
            message: 'scene review configuration lenses must be a canonical subset ordered layout, geometry, final',
        });
    }
    const reviewFollowsConfiguration = review.lenses.length === configuration.lenses.length &&
        review.lenses.every((lens, index) => lens.id === configuration.lenses[index]);
...
function collectRuntimeNodeIds(document) {
    const ids = new Set();
    const prefabs = new Map((document.resources.prefabs ?? []).map((prefab) => [prefab.id, prefab]));
    const visit = (nodes, namespace, prefabStack) => {
        nodes.forEach((node) => {
...
function patternInstanceUpperBound(distribution) {
...
function cameraMatchesView(camera, view) {
    return (camera.projection === view.projection &&
...
function compareHash(issues, path, actual, expected) {
...
function addReference(issues, path, message) {
    issues.push({ code: 'reference', path, message });
}
function resultKey(feature, criterion) {
    return `${feature}\u0000${criterion}`;
}

export { finalizeSceneReviewDraft, validateSceneReviewAgainstDocument };
