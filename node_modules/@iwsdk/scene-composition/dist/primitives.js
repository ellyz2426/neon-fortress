 ...
 */
const DEFAULT_SCENE_PRIMITIVE_SEGMENTS = 24;
const MIN_SCENE_PRIMITIVE_SEGMENTS = 3;
const MAX_SCENE_PRIMITIVE_SEGMENTS = 64;
function getScenePrimitiveBounds(primitive) {
    switch (primitive.type) {
        ...
        case 'sphere':
            return centeredBounds(primitive.radius, primitive.radius, primitive.radius);
        case 'cylinder': {
            const radius = 'radius' in primitive
                ? primitive.radius
                : Math.max(primitive.radiusTop, primitive.radiusBottom);
            return centeredBounds(radius, primitive.height / 2, radius);
        }
        case 'cone':
            return centeredBounds(primitive.radius, primitive.height / 2, primitive.radius);
        case 'plane': {
            const [width, height] = primitive.size;
            return centeredBounds(width / 2, height / 2, 0);
        }
        case 'capsule':
            return centeredBounds(primitive.radius, primitive.length / 2 + primitive.radius, primitive.radius);
        case 'extrude': {
        ...
        case 'lathe': {
            const radius = Math.max(...primitive.profile.map((point) => Math.abs(point[0])));
            return {
                min: [
                    radius === 0 ? 0 : -radius,
...
function negateWithoutNegativeZero(value) {
    return value === 0 ? 0 : -value;
}

export { DEFAULT_SCENE_PRIMITIVE_SEGMENTS, MAX_SCENE_PRIMITIVE_SEGMENTS, MIN_SCENE_PRIMITIVE_SEGMENTS, getSceneNodeLocalBounds, getScenePrimitiveBounds };
